...
当前整理 **291 个插件**，统一归入七个大类。每个插件只出现一次，并按照最主要的使用价值进行分类。最近一次目录复核：**2026 年 9 月 22 日**。
...
- [dsh-workspace-scope-selection](https://github.com/jiangr100/dsh-workspace-scope-selection) — 为 DeepSeek Harness Web 会话添加所选工作区写入权限模式和目录树范围编辑器。
...
- [dsh-dry-guide](https://github.com/uckkk/dsh-dry-guide) — 根据输入参数提供晾晒烘干建议。
...
- [dsh-spirit-guide](https://github.com/uckkk/dsh-spirit-guide) — 提供一个 spirit_guide 工具，根据参数返回威士忌和烈酒推荐。
...
- [dsh-dsml-artifact-guard](https://github.com/GooDAnDReaDY/dsh-dsml-artifact-guard) — 通过同步的 llm/stream 钩子清理 DeepSeek Harness 模型输出流中泄漏的 DSML 协议闭合标签。
- [dsh-context-lens](https://github.com/GooDAnDReaDY/dsh-context-lens) — 一个 DeepSeek Harness 插件，提供基于 AST 的代码骨架化、日志压缩、聚焦路径作用域以及会话 token 节省统计与预算配置工具。
- [dsh-approval-gate](https://github.com/GooDAnDReaDY/dsh-approval-gate) — 一个 DeepSeek Harness 宿主插件，注册原生 tools.guard，在工具执行前拦截破坏性 shell 命令与受保护文件写入，并将无法确定的语法交由 DSH 审批流程处理。
...
- [dsh-resume-expert](https://github.com/wmw343/dsh-resume-expert) — 一个 DeepSeek Harness 插件，通过多轮对话引导用户逐步完成简历撰写，并导出单页 A4 PDF。
...
- [deepSea](https://github.com/evil7/deepSea) — 单体仓库，包含 DSH 插件生态搜索站点与 deepc-link 插件；该插件安装到 DSH 后通过 Cloudflare Tunnel 暴露本地 DSH 主机，用于远程控制与数据同步。
