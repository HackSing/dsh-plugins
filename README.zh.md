...
当前整理 **268 个插件**，统一归入七个大类。每个插件只出现一次，并按照最主要的使用价值进行分类。最近一次目录复核：**2026 年 9 月 11 日**。
...
这是目录的首个公开快照：共 **268 个插件**、**7 个大类**，中英文条目一一对应。后续新增、移除和修正统一记录在 [CHANGELOG.md](CHANGELOG.md)。
...
- [dsh-clinebot](https://github.com/GooDAnDReaDY/dsh-clinebot) — 提供一个 DeepSeek Harness 插件，将 Harness 连接到 ClineBot/ClinePass 的 OpenAI 兼容 API 端点，并添加设置页面、模型同步、配额显示、界面内密钥存储以及 /cline 斜杠命令。
