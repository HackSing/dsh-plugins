...
当前整理 **257 个插件**，统一归入七个大类。每个插件只出现一次，并按照最主要的使用价值进行分类。最近一次目录复核：**2026 年 9 月 9 日**。
...
这是目录的首个公开快照：共 **257 个插件**、**7 个大类**，中英文条目一一对应。后续新增、移除和修正统一记录在 [CHANGELOG.md](CHANGELOG.md)。
...
- [dsh-key-rotation](https://github.com/GooDAnDReaDY/dsh-key-rotation) — 为 DeepSeek Harness 提供按提供商分配的 API 密钥轮换、自动速率限制故障转移和冷却探测功能。
