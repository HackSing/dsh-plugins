...
当前整理 **265 个插件**，统一归入七个大类。每个插件只出现一次，并按照最主要的使用价值进行分类。最近一次目录复核：**2026 年 9 月 10 日**。
...
这是目录的首个公开快照：共 **265 个插件**、**7 个大类**，中英文条目一一对应。后续新增、移除和修正统一记录在 [CHANGELOG.md](CHANGELOG.md)。
...
- [dsh-dsml-artifact-guard](https://github.com/GooDAnDReaDY/dsh-dsml-artifact-guard) — 提供一个 DeepSeek Harness 插件，通过挂接 llm/stream 事件对助手文本中的 DSML 闭合标签进行缓冲与清除，使其不会进入聊天界面。
