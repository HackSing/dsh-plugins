...
当前整理 **264 个插件**，统一归入七个大类。每个插件只出现一次，并按照最主要的使用价值进行分类。最近一次目录复核：**2026 年 9 月 10 日**。
...
这是目录的首个公开快照：共 **264 个插件**、**7 个大类**，中英文条目一一对应。后续新增、移除和修正统一记录在 [CHANGELOG.md](CHANGELOG.md)。
...
- [dsh-gitea](https://github.com/GooDAnDReaDY/dsh-gitea) — DeepSeek Harness 插件，将 Gitea 与 Forgejo 实例的议题、拉取请求、CI、发布、标签、Webhook 与 git worktree 操作以代理工具形式提供。
